...
class OrdersFinalPaymentMapper extends BaseMapper
{
    ...
    public function finalPaymentRecWithOrderSn($where) {
        return Db::table("orders_final_payment")->alias("od")
            ...
            ->where($where)
            ->where(["or.is_split" => 0])
            ->field("o.origin_name, om.customer_contact, om.order_account_id, aa.account, od.change_amount, or.order_sn, od.create_time, u.name, u1.name as customer_name, or.require, or.create_time as order_create_time, or.status")
            ->select()->toArray();
    }
}
